...
import {
  ...
  FaHome, FaSearch, FaSignOutAlt, FaStore, FaTag, FaUser, FaUsers, FaReceipt,
...
const ShiftLog = lazy(() => import("./ShiftLog"));
const CouponManager = lazy(() => import("./CouponManager"));
const ReceiptCustomizer = lazy(() => import("./ReceiptCustomizer"));
const InventoryManager = lazy(() => import("./InventoryManager"));
const TransactionsLedger = lazy(() => import("./TransactionsLedger"));
const InventoryControl = lazy(() => import("./InventoryControl"));
...
function Dashboard() {
    ...
    const fetchStats = async () => {
      try {
        ...
        const [usersRes, productsRes, salesRes, analyticsRes] = await Promise.all([
          ...
          axios.get(`${config.apiUrl}/api/analytics`, { headers }),
        ]);
  ...
  const operationsItems = [
    ...
    { text: "Suppliers", path: "/dashboard/inventory-manager", icon: <FaBox /> },
    { text: "Inventory control", path: "/dashboard/inventory-control", icon: <FaBoxes /> },
    { text: "Returns & voids", path: "/dashboard/returns", icon: <FaExchangeAlt /> },
    { text: "Reconciliation", path: "/dashboard/reconciliation", icon: <FaBalanceScale /> },
    { text: "Transactions", path: "/dashboard/transactions-ledger", icon: <FaReceipt /> },
    { text: "Audit trail", path: "/dashboard/audit-trail", icon: <FaHistory /> },
  ];
  ...
  return <div className="min-h-screen bg-slate-50 flex font-sans">
    ...
    <div className={`min-w-0 flex-1 ${isMobile ? "" : "ml-[268px]"}`}>
      ...
      <main className="max-w-[1440px] mx-auto p-5 md:p-8">
        ...
        <section className={isOverview ? "mt-7" : ""}><Routes><Route path="user-list" element={<UserList/>}/><Route path="product-list" element={<ProductList/>}/><Route path="total-sales" element={<TotalSales/>}/><Route path="admin-profile" element={<AdminProfile/>}/><Route path="shifts-log" element={<ShiftLog/>}/><Route path="coupon-manager" element={<CouponManager/>}/><Route path="receipt-customizer" element={<ReceiptCustomizer/>}/><Route path="inventory-manager" element={<InventoryManager/>}/><Route path="inventory-control" element={<InventoryControl/>}/><Route path="returns" element={<ReturnsManager/>}/><Route path="reconciliation" element={<ReconciliationReports/>}/><Route path="transactions-ledger" element={<TransactionsLedger/>}/><Route path="audit-trail" element={<AuditTrail/>}/></Routes></section>
      </main>
    </div>
  </div>;
}
